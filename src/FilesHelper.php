...
class FilesHelper
{
    ...
    static public function file_format($file){
        ...
        // 图片格式
        $image=['webp','jpg','png','ico','bmp','gif','tif','pcx','tga','bmp','pxc','tiff','jpeg','exif','fpx','svg','psd','cdr','pcd','dxf','ufo','eps','ai','hdri'];
        // 视频格式
        $video=['mp4','avi','3gp','rmvb','gif','wmv','mkv','mpg','vob','mov','flv','swf','mp3','ape','wma','aac','mmf','amr','m4a','m4r','ogg','wav','wavpack'];
        // 压缩格式
        $zip=['rar','zip','tar','cab','uue','jar','iso','z','7-zip','ace','lzh','arj','gzip','bz2','tz'];
        // 文档格式
        $text=['exe','doc','ppt','xls','wps','txt','lrc','wfs','torrent','html','htm','java','js','css','less','php','pdf','pps','host','box','docx','word','perfect','dot','dsf','efe','ini','json','lnk','log','msi','ost','pcs','tmp','xlsb'];
        // 匹配不同的结果
        switch ($str) {
            case ArrayHelper::in_iarray($str, $image):
                $result = [
                    'file_type'=>'image',
                    'file_ext' =>$str,
                ];
                break;
            case ArrayHelper::in_iarray($str, $video):
                $result = [
                    'file_type'=>'video',
                    'file_ext' =>$str,
                ];
                break;
            case ArrayHelper::in_iarray($str, $zip):
                $result = [
                    'file_type'=>'zip',
                    'file_ext' =>$str,
                ];
                break;
            case ArrayHelper::in_iarray($str, $text):
                $result = [
                    'file_type'=>'text',
                    'file_ext' =>$str,
                ];
